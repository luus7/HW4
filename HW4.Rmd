...
#wasn't working at first it was just because "labs" was "lab"
```
...
See red point located on graph above. 
...
```{r}
warming_data$warming_B <- ifelse(warming_data$year>=2050, 1.29, warming_data$warming_baseline)

damages_B <- data.frame(predict_damage = predict(quad_dmg, newdata=tibble(warming=warming_data$warming_B))) %>% mutate(years=warming_data$year)
...
damage_B_total <- sum(damages_B$predict_damage)/(1+.02)^(years_B[i]-2023)
df_damage_B <- data.frame(damage_B_total)
df_dmgs_B <- rbind(df_dmgs_B,df_damage_B)
}
df_dmgs_B

sum(df_dmgs_B$damage_B_total)
