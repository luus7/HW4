...
for(j in 1:length(years)){
     future_discounted_dmg <- diff_damage_per_ton$diff_dmg/(1+i)^(j-2023)
 }
   df_discount <- data.frame("rates" = i, future_discounted_dmg)
   df_dmgs <- rbind(df_dmgs,df_discount)
}
 df_dmgs %>%
   group_by(rates) %>%
   summarize(sum(future_discounted_dmg))
...
for(i in 1:length(discount)){
for(j in 1:length(years)){
     future_discounted_dmg <- sum(diff_damage_per_ton$diff_dmg)/(1+discount[i])^(years[j]-2023)
 }
